  ...
  return (
    <main className="min-h-screen bg-dark text-white">
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <p className="mt-2 text-white/70">Mock portfolio. Displaying AI confidence and margin ranges.</p>
        <div className="mt-6 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {brands.map((b) => (
            <div key={b.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <div className="text-sm text-white/70">Est. net margin 10–20% · {b.status} · {b.investors} backers</div>
            </div>
          ))}
        </div>
      </div>
